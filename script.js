...
define(['jquery', 'underscore', 'twigjs'], function ($, _, Twig) {
    var CustomWidget = function () {
        ...
        // количество символов для создания задачи
        this.create_task_length = null;

        // вытаскиваем данные из системной переменной
        this.toSettings = function () {
            self.checkboxes = self.get_settings().checkboxes_create || {};
        ...
        this.lastSettings = function (checkboxes) {
            ...
            $('.create_task_checkbox_wrapper').after(linkUsers);
        ...
        this.callbacks = {
            settings: function () {
                ...
                self.createTaskToNumber();
                $('input[name="create_task"]').val(self.create_task_length);

                // если виджет не установлен, обнуляем значение из настроек
                if (self.get_install_status() === 'not_configured') {
                    self.set_settings({ create_task: 0 });
                    self.create_task_length = null;
                    $('input[name="create_task"]').val(0);
                }
            ...
            render: function () {
                ...
                if (self.create_task_length === null) self.createTaskToNumber();
